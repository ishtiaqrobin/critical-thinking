//* Binning (Resampling) Time Series Data

// Senario: You have a long list of user click events.
// You need to "bin" these events into 30-minute intervals and count them to see engagement over time.

//? Input 
const events = [
    { timestamp: "2025-10-22T10:01:00Z", type: "click" },
    { timestamp: "2025-10-22T10:05:00Z", type: "scroll" },
    { timestamp: "2025-10-22T10:14:00Z", type: "clcik" },
    { timestamp: "2025-10-22T10:31:00Z", type: "clcik" },
    { timestamp: "2025-10-22T10:45:00Z", type: "scroll" },
    { timestamp: "2025-10-22T11:02:00Z", type: "clcik" },
];

const INTERVAL = 30 * 60 * 1000; // 30 minutes in milliseconds

const getBanningTimeStamp = (timestamp) => {
    const date = new Date(timestamp);

    // console.log("Before", date.toISOString());

    const flooredDate = Math.floor(date.getTime() / INTERVAL) * INTERVAL;

    return new Date(flooredDate).toISOString();
}

const binnedDate = events.reduce((acc, event) => {
    const bin = getBanningTimeStamp(event.timestamp);

    if (!acc[bin]) {
        acc[bin] = { total: 0 }
    }

    acc[bin].total = acc[bin].total + 1;

    return acc
}, {})

console.log(binnedDate);

//? Output
// binnedEvents ={
//    "2025-10-22T10:00:00:00Z": {"total": 3},
//    "2025-10-22T10:30:00:00Z": {"total": 2},
//    "2025-10-22T11:00:00:00Z": {"total": 1},
// }